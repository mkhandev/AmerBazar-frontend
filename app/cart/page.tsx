  ...
  return (
    <>
      {!cart || !cart.data || cart.data.length === 0 ? (
        ...
        <div className="grid md:grid-cols-5 md:gap-5 mt-5">
          <div className="md:col-span-4">
            {cart.data.map((item: any) => {
              ...
              return (
                <div
                  className="grid grid-cols-5 p-3 bg-white mt-3 rounded-lg shadow-sm"
                  key={item.id}
                >
                  {/* Product Image */}
                  <div>
          ...
          {/* --- Cart Summary --- */}
          <div className="p-5 bg-white rounded-lg shadow-sm h-fit">
            ...
            <div className="flex justify-between mt-3 border-t pt-3 font-bold">
              <span>Grand Total</span>
              <span>${grandTotal.toFixed(2)}</span>
            </div>

            <button className="mt-5 w-full bg-[#37a001] text-white py-3 rounded-lg hover:bg-green-700 transition">
              Proceed to Checkout
            </button>
          </div>
        </div>
